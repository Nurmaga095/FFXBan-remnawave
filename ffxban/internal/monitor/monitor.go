...
type redisHousekeeper interface {
	CleanupOrphanMappings(ctx context.Context, maxKeys int, enforceTTL time.Duration) (scanned int, deleted int, ttlRepaired int, err error)
}

// NewPoolMonitor создает новый экземпляр PoolMonitor.
func NewPoolMonitor(s storage.IPStorage, limitProvider panel.UserLimitProvider, cfg *config.Config) *PoolMonitor {
	return &PoolMonitor{
		storage:       s,
...
func (m *PoolMonitor) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	var housekeepingCh <-chan time.Time
	if m.cfg.RedisHousekeepingEnabled {
		housekeepingTicker = time.NewTicker(m.cfg.RedisHousekeepingInterval)
		housekeepingCh = housekeepingTicker.C
		defer housekeepingTicker.Stop()
		log.Printf("Redis housekeeping запущен с интервалом %v", m.cfg.RedisHousekeepingInterval)
	}
	...
	for {
		...
		case <-housekeepingCh:
			m.performRedisHousekeeping(context.Background())
		case <-ctx.Done():
...
func (m *PoolMonitor) performRedisHousekeeping(ctx context.Context) {
	housekeeper, ok := m.storage.(redisHousekeeper)
	if !ok {
		return
	}

	scanned, deleted, ttlRepaired, err := housekeeper.CleanupOrphanMappings(ctx, m.cfg.RedisHousekeepingMaxKeys, m.cfg.UserIPTTL)
	if err != nil {
		log.Printf("Ошибка Redis housekeeping: %v", err)
		return
	}

	if scanned > 0 || deleted > 0 || ttlRepaired > 0 {
		log.Printf("Redis housekeeping: scanned=%d deleted=%d ttl_repaired=%d", scanned, deleted, ttlRepaired)
	}
}
...
func (m *PoolMonitor) getUserIPLimit(userEmail string) int {
	...
	if m.limitProvider != nil {
		if limit, ok := m.limitProvider.GetUserLimit(userEmail); ok && limit > 0 {
			return limit
		}
	}
