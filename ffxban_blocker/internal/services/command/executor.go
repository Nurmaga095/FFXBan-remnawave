...
func NewExecutor(l *logger.Logger, nftBinary string) *Executor {
	return &Executor{
		logger:    l,
		nftBinary: nftBinary,
	}
...
func (e *Executor) RunNftCommand(ctx context.Context, args ...string) error {
	// Добавляем бинарник nft в начало списка аргументов.
	fullArgs := append([]string{e.nftBinary}, args...)
...
// ValidateTarget проверяет доступность целевого набора в nftables.
func (e *Executor) ValidateTarget(ctx context.Context, family, table, set string) error {
	return e.RunNftCommand(ctx, "list", "set", family, table, set)
}
